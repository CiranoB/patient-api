...
public class PatientController {
    ...
    @Operation(summary = "Adicionar registro médico ao paciente", description = "Adiciona um novo registro ao paciente identificado pelo userName. Se o paciente não existir, um novo será criado.")
    public ResponseEntity<Patient> addRecordToPatient(
            @RequestBody PatientRecord newRecord) {
        return ResponseEntity.ok(this.patientService.addRecordToPatient(newRecord));
    }

    @PostMapping("/check_urgent_care_need")
    @Operation(summary = "Verifica a necessidade de atendimento urgente", description = "Com base no formulário inicial preenchido pelo paciente, este endpoint analisa a necessidade de atendimento imediato e recomenda uma especialidade médica apropriada.")
    public ResponseEntity<String> checkUrgentCareNeed(
            @RequestBody UserInitialFormDTO message) {
        return ResponseEntity.ok(this.iaService.runGemini(
    ...
    @Operation(summary = "Obter resumo dos registros médicos", description = "Busca os registros médicos de um paciente e gera um resumo que será lido por um médico posteriormente.")
    public ResponseEntity<String> getRecordsSummary(
        ...
        String summary = this.iaService.runGemini(
